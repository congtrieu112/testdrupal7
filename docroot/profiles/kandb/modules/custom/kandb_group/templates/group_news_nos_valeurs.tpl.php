...
$sub_title = variable_get('news_values_module_sub_title');
?>
...
<section class="wrapper section-padding ourAdvices">
    ...
    <?php for ($i = 0; $i <= KANDB_GROUP_NEWS_ITEMS_NUM; $i++) : ?>
      <?php
      $title = variable_get('news_values_item_title_' . $i);
      $description = variable_get('news_values_item_description_' . $i);
      $description = isset($description['value']) ? $description['value'] : '';
      $image = variable_get('news_values_item_image_' . $i);
